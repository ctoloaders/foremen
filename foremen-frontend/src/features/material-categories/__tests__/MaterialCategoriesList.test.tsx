    ...
    useTranslation: () => ({
      t: (key: string) => {
        const translations: Record<string, string> = {
          'materialCategories.actions.create': 'Utwórz kategorię materiału',
          'materialCategories.table.code': 'Kod',
          'materialCategories.table.name': 'Nazwa',
          'materialCategories.table.active': 'Aktywna',
          'materialCategories.badge.active': 'Aktywna',
          'materialCategories.badge.inactive': 'Nieaktywna',
          'common.edit': 'Edytuj',
          'common.delete': 'Usuń',
          'dataTable.search': 'Szukaj...',
          'dataTable.empty': 'Nie znaleziono kategorii materiałów',
...
const mockOnCreateMaterialCategory = vi.fn()
const mockOnEditMaterialCategory = vi.fn()
const mockOnDeleteMaterialCategory = vi.fn()
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <MaterialCategoriesList
        onCreateMaterialCategory={mockOnCreateMaterialCategory}
        onEditMaterialCategory={mockOnEditMaterialCategory}
        onDeleteMaterialCategory={mockOnDeleteMaterialCategory}
      />
    </QueryClientProvider>,
  )
}

const sampleMaterialCategories: MaterialCategoryDto[] = [
  { id: 1, code: 'construction', name: 'Construction', active: true },
  { id: 2, code: 'finishing', name: 'Finishing', active: true },
  { id: 3, code: 'other', name: 'Inne', active: false },
]

function mockFetchSuccess(materialCategories: MaterialCategoryDto[] = sampleMaterialCategories) {
  vi.spyOn(globalThis, 'fetch').mockResolvedValue(
    new Response(
      JSON.stringify({
        content: materialCategories,
        totalPages: 1,
        totalElements: materialCategories.length,
        number: 0,
        size: 10,
      }),
...
describe('MaterialCategoriesList', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockUseBreakpoint.mockReturnValue('desktop')
    localStorage.setItem('foremen-locale', 'pl')
    // FOR-03-07 gates the Create/Edit/Delete actions behind MATERIAL_CATEGORIES
  ...
  describe('Permission-gated Create button', () => {
    it('"Create" button is rendered and clickable with CREATE permission', async () => {
      ...
      const createButton = screen.getByText('Utwórz kategorię materiału')
      expect(createButton).toBeInTheDocument()

      fireEvent.click(createButton)
      expect(mockOnCreateMaterialCategory).toHaveBeenCalledTimes(1)
    })
  })
  ...
  describe('Row actions', () => {
    it('renders edit and delete buttons for every row with CRUD permissions', async () => {
      ...
      expect(editButtons.length).toBe(sampleMaterialCategories.length)
      expect(deleteButtons.length).toBe(sampleMaterialCategories.length)
    })
  })

  describe('Empty state', () => {
    it('shows empty state message when no material categories', async () => {
      mockFetchSuccess([])
